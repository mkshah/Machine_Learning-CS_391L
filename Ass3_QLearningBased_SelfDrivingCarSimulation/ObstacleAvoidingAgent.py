...
class ObstacleAvoidingAgent():
	...
	def get_Best_Action(self):
		...
		for action in possible_actions:
			a = 1 if self.environment.isObstacleNearby(self.environment.state) else 0
	...
	def do_Action(self):
		...
		reward = self.environment.get_Reward_ObstacleAvoidingAgent(prev_state, action)
	...
	def update_Qvalues(self, state, action, reward):
		a = 1 if self.environment.isObstacleNearby(state) else 0
